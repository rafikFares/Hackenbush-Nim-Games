...
public class Nim {
    ...
    public void setNimType(boolean normal) {
        this.normal = normal;
    }
    ...
    public static boolean isFinished(int[] heap) { // si ya plus de tige // le jeu est fini
        for (int i = 0; i < heap.length; i++) {
            if (heap[i] > 0) {
                return false;
            }
    ...
    public Move nextMove(int[] heap) {// travaille avec ca qui va générer un move optimal si ya // sinon elle genre un move random
    ...
    public static void applyMove(Move move, int[] heap) {// applique le move sur le tableau heap
        System.out.println("BEFORE ");
        printNim(heap);
        if (move == Move.EMPTY) {
            return;
        }
        if (heap[move.getIndex()] <= move.getSize()) {
            throw new IllegalArgumentException("invalid " + move.toString());
        }
        heap[move.getIndex()] = move.getSize();
    ...
    static public class Move {//index est le i // size est combien d'arret doivent rester dans la tige i
